...
function StatCard({ icon: Icon, label, value, color, subtext }: {
  ...
  return (
    <div className="glass rounded-xl p-5 hover:border-accent-500/30 transition-all group">
    ...
    <Sidebar>
      <div className="p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4 mb-8">
          <StatCard icon={Map} label="Total Parcels" value={stats?.total_parcels?.toString() || '0'} color="bg-accent-600" />
          <StatCard icon={Building2} label="Buildings" value={stats?.total_buildings?.toString() || '0'} color="bg-success-700" />
          <StatCard icon={Target} label="Mapped Area" value={stats?.total_mapped_area ? formatArea(stats.total_mapped_area) : '0'} color="bg-accent-500" />
          <StatCard icon={Route} label="Road Length" value={stats?.total_road_length ? `${(stats.total_road_length / 1000).toFixed(1)} km` : '0'} color="bg-warning-500" />
          <StatCard icon={Target} label="AI Accuracy" value={stats?.average_confidence ? `${(stats.average_confidence * 100).toFixed(0)}%` : '0%'} color="bg-success-500" />
          <StatCard icon={Clock} label="Processing" value={stats?.processing_time ? `${stats.processing_time}s` : '0s'} color="bg-navy-600" />
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="glass rounded-xl p-6">
            ...
            <div className="space-y-3">
              <Link to="/map" className="flex items-center gap-3 p-3 rounded-lg bg-navy-800/50 hover:bg-navy-700/50 border border-navy-700/30 transition-all group">
                <Map className="w-5 h-5 text-accent-400" />
                <div className="flex-1">
                  <p className="text-sm font-medium">Interactive Map</p>
                  <p className="text-xs text-navy-400">Explore detected features</p>
                </div>
                <ArrowRight className="w-4 h-4 text-navy-400 group-hover:text-accent-400 transition-colors" />
              </Link>
              <Link to="/analysis" className="flex items-center gap-3 p-3 rounded-lg bg-navy-800/50 hover:bg-navy-700/50 border border-navy-700/30 transition-all group">
                <BarChart3 className="w-5 h-5 text-success-500" />
                <div className="flex-1">
                  <p className="text-sm font-medium">AI Analysis</p>
                  <p className="text-xs text-navy-400">Run detection pipeline</p>
                </div>
                <ArrowRight className="w-4 h-4 text-navy-400 group-hover:text-success-500 transition-colors" />
              </Link>
              <Link to="/export" className="flex items-center gap-3 p-3 rounded-lg bg-navy-800/50 hover:bg-navy-700/50 border border-navy-700/30 transition-all group">
                <Upload className="w-5 h-5 text-warning-500" />
                <div className="flex-1">
                  <p className="text-sm font-medium">Export Data</p>
                  <p className="text-xs text-navy-400">Download GIS formats</p>
                </div>
                <ArrowRight className="w-4 h-4 text-navy-400 group-hover:text-warning-500 transition-colors" />
              </Link>
              <Link to="/reports" className="flex items-center gap-3 p-3 rounded-lg bg-navy-800/50 hover:bg-navy-700/50 border border-navy-700/30 transition-all group">
                <TreePine className="w-5 h-5 text-accent-300" />
                <div className="flex-1">
                  <p className="text-sm font-medium">Generate Report</p>
                  <p className="text-xs text-navy-400">Create cadastral report</p>
                </div>
                <ArrowRight className="w-4 h-4 text-navy-400 group-hover:text-accent-300 transition-colors" />
              </Link>
            </div>
          </div>
        </div>
